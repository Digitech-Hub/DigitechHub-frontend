...
export default function useBottomNavigation() {
  ...
  // 반원 형태 위치 계산
  const getMenuItemPosition = useCallback((index: number, totalItems: number, radius: number) => {
  ...
  return {
    ...
    MENU_ITEMS,
    getMenuItemPosition,
    isItemVisible: useCallback((itemId: number) => visibleItems.has(itemId), [visibleItems])
  };
}
