...
export default function Navigation() {
  ...
  const { isOpen, toggleNavigation, MENU_ITEMS, getMenuItemPosition, isItemVisible } = useBottomNavigation();
  ...
  return (
    <div className="fixed left-1/2 bottom-4 transform -translate-x-1/2">
        ...
        <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2">
          <div className="relative">
            {MENU_ITEMS.map((item, index) => {
              const { x, y } = getMenuItemPosition(index, MENU_ITEMS.length, radius);
              const isVisible = isItemVisible(item.id);

              return (
                <div
                  key={item.id}
                  className={`absolute transform -translate-x-1/2 -translate-y-1/2 transition-all duration-500 ease-out ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-50'
                    }`}
                ...
                >
                  ...
                  <div className={`
                    ...
                    ${isDark ? "bg-gray-800 text-gray-200 border border-gray-600" : "bg-gray-900 text-white"}
                  `}>
                    {item.label}
                  </div>
                </div>
      ...
      <nav
        className={`
          ${isDark ? "bg-gray-800 border border-gray-200" : "bg-white border border-gray-200"} 
      ...
      >
        <div className="flex items-center justify-center w-14 h-14 rounded-full bg-gradient-to-r from-red-500 via-purple-500 to-blue-500 animate-gradient-shift bg-[length:200%_200%]">
          <div className={`w-6 h-6 duration-300 rounded-full transition-transform ${isDark ? "bg-gray-800" : "bg-white"}`} />
        </div>
      </nav>
    </div>
  );
